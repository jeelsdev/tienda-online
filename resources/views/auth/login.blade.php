          ...
          <div class="flex-auto p-6">
            <form role="form" method="POST" action="{{ route('login') }}">
              ...
              <div class="text-center">
                <button type="submit" class="inline-block w-full px-16 py-3.5 mt-6 mb-0 font-bold leading-normal text-center text-white align-middle transition-all bg-pink-500 border-0 rounded-lg cursor-pointer hover:-translate-y-px active:opacity-85 hover:shadow-xs text-sm ease-in tracking-tight-rem shadow-md bg-150 bg-x-25">Sign in</button>
              </div>
            </form>
          </div>
          <div class="border-black/12.5 rounded-b-2xl border-t-0 border-solid p-6 text-center pt-0 px-1 sm:px-6">
            <p class="mx-auto mb-2 leading-normal text-sm">No tienes una cuenta? <a href="{{ route('register') }}" class="font-semibold text-transparent bg-clip-text bg-gradient-to-tl from-pink-500 to-violet-500">Registrarte</a></p>
            <a href="{{ route('password.request') }}" class="font-semibold text-transparent bg-clip-text bg-gradient-to-tl from-pink-500 to-violet-500">Olvidadeste tu contraseña?</a>
          </div>
        </div>
      </div>
