    ...
    <div class="flex flex-col w-full max-w-full px-3 mx-auto lg:mx-0 shrink-0 md:flex-0 md:w-7/12 lg:w-5/12 xl:w-4/12">
        <div class="relative flex flex-col min-w-0 break-words bg-transparent border-0 shadow-none lg:py4  rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0">
            <h4 class="font-bold">Registrarse como vendedor</h4>
          </div>
          <div class="flex-auto p-6">
            <form method="POST" action="{{ route('register-staff') }}">
              ...
              <div class="mb-4">
                <x-input type="text" id="surname" name="surnames" value="{{ old('surname') }}"  placeholder="Apellidos"></x-input>
                @error('surnames')
                <span class="text-red-500">{{ $message }}</span>
               @enderror
              </div>
          ...
          <div class="border-black/12.5 rounded-b-2xl border-t-0 border-solid p-6 text-center pt-0 px-1 sm:px-6">
            <p class="mx-auto mb-2 leading-normal text-sm">Ya tienes una cuenta? <a href="{{ route('login') }}" class="font-semibold text-transparent bg-clip-text bg-gradient-to-tl from-blue-500 to-violet-500">Inciar sesión</a></p>
            <p class="mx-auto mb-2 leading-normal text-sm">Quieres solo comprara productos? <a href="{{ route('register') }}" class="font-semibold text-transparent bg-clip-text bg-gradient-to-tl from-blue-500 to-violet-500">Registrate como cliente</a></p>
            </div>
        </div>
      </div>
      <div class="absolute top-0 right-0 flex-col justify-center hidden w-6/12 h-full max-w-full px-3 pr-0 my-auto text-center flex-0 lg:flex">
        <div class="relative flex flex-col justify-center h-full bg-cover px-24 m-4 overflow-hidden bg-[url('https://raw.githubusercontent.com/creativetimofficial/public-assets/master/argon-dashboard-pro/assets/img/signin-ill.jpg')] rounded-xl ">
          ...
          <h3 class="z-20 mt-12 font-extrabold text-white">¡Bienvenido a bordo vendedor!</h3>
          <p class="z-20 text-white ">¿Tienes una tienda?, Regístrate y comienza a vender con nosotros...</p>
        </div>
      </div>
